...
const NeuralNoise: ComponentType<NeuralNoiseProps> = (props) => {
    ...
    useEffect(() => {
        ...
        colorUniformsRef.current = {
            colorA: colorALocation,
            colorB: colorBLocation,
        }
        ...
        const drawFrame = (now: number): boolean => {
            ...
            const shouldAnimate = preview || !isCanvas
            const effectiveNow = shouldAnimate ? now : startTime
            ...
            const elapsedSeconds =
                (effectiveNow - startTime) * 0.001 * mappedAnimationSpeed
        ...
        const updatePointer = (event: PointerEvent) => {
            ...
            if (RenderTarget.current() === RenderTarget.canvas && !preview) {
                resizeCanvas()
...
addPropertyControls(NeuralNoise, {
    preview: {
...
NeuralNoise.displayName = "Neural Noise"
