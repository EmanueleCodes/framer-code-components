...
function isImageCanvasSafe(img: HTMLImageElement) {
    ...
    try {
        ...
        return !!crossOrigin && crossOrigin.length > 0
    } catch (error) {
        return false
    }
}

function elementContainsUnsafeImages(element: Element) {
    if (element instanceof HTMLImageElement) {
        return !isImageCanvasSafe(element)
    }
...
async function convertImageToDataURL(src: string) {
    ...
    if (!response.ok) {
        throw new Error(`CRT Effect: Unable to fetch image (${response.status})`)
    }
...
async function prepareElementForCapture(
    element: Element
): Promise<PreparedElement | null> {
    if (!elementContainsUnsafeImages(element)) {
        return {
            element,
            restore: () => undefined,
        }
    ...
    if (element instanceof HTMLImageElement) {
        ...
        images.push(...Array.from(element.querySelectorAll("img")))
    }

    const replacements: Array<{ img: HTMLImageElement; originalSrc: string }> =
        []

    for (const img of images) {
        if (isImageCanvasSafe(img)) continue
        const src = img.getAttribute("src")
        if (!src) continue
        try {
            const dataUrl = await convertImageToDataURL(src)
            replacements.push({ img, originalSrc: src })
            img.setAttribute("data-crt-original-src", src)
            img.src = dataUrl
        } catch (error) {
            replacements.forEach(({ img: originalImg, originalSrc }) => {
                originalImg.src = originalSrc
                originalImg.removeAttribute("data-crt-original-src")
            })
            return null
        }
    }
    ...
    return {
        ...
        restore: () => {
            replacements.forEach(({ img, originalSrc }) => {
                if (img.getAttribute("data-crt-original-src")) {
                    img.src = originalSrc
                    img.removeAttribute("data-crt-original-src")
                }
            })
        },
    }
...
export default function CRTComponent(props: CRTComponentProps) {
    ...
    useEffect(() => {
        ...
        const imageRestoreCallbacks: Array<() => void> = []
        let addCancelled = false

        const prepareAndAddElements = async () => {
            const elementsToCapture = Array.from(
                targetElement.querySelectorAll(targetElements)
            )

            const filteredElements = elementsToCapture.filter((element) => {
                return !elementsToCapture.some(
            ...
            const preparedElements = await Promise.all(
                filteredElements.map(async (element) => {
                    if (!isMountedRef.current || addCancelled)
                        return { prepared: null, element }
                    const result = await prepareElementForCapture(element)
                    return { prepared: result, element }
                })
            )

            if (!isMountedRef.current || addCancelled) {
                preparedElements.forEach((prepared) => {
                    prepared.prepared?.restore()
                })
                return
            }
            ...
            preparedElements.forEach((prepared) => {
                if (!prepared.prepared) {
                    console.warn(
                        "CRT Effect: Skipping element with images that cannot be captured. Ensure images allow CORS or use same-origin assets.",
                        prepared.element
                    )
                    return
                }
                const { element, restore } = prepared.prepared
                imageRestoreCallbacks.push(restore)
                try {
        ...
        return () => {
            addCancelled = true
            imageRestoreCallbacks.forEach((restore) => {
                try {
                    restore()
                } catch (e) {
            ...
            imageRestoreCallbacks.length = 0
